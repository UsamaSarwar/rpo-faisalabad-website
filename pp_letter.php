...
<div class="page-content">
 ...
 <span style="color: #184d88;">Progress </span> |<span style="font-size: 14px; font-weight: 300;">Pending Letters</span></div>
...
<?php
		$counter = 0;
	$selectp = "SELECT * FROM progress WHERE status = 'Pending'";
	...
	$pid = $datap['dispatch_number'];		
			
	$selectsp = "SELECT * FROM progress_sub WHERE progress_id = '$pid'";
	$runsp = mysqli_query($conn,$selectsp);
	$datasp = mysqli_fetch_array($runsp);
	$datapid = $datasp['progress_id'];
			?>
						<tr>
                    <td><?php echo ++$counter; ?></td>
                    <td><?php
						if(empty($datapid)){	
						echo $datap['dispatch_number']; 
						}else{
							?>
						<a href="ppm_letter.php?id=<?php echo $datasp['progress_id'];?>"><?php echo $datap['dispatch_number'];?></a>
						<?php
						}
...
<div class="dropdown dropdown-status classDropup ">
				...
				<div class="dropdown-menu">
					<a class="dropdown-item" href="pm_letter.php?id=<?php echo $datap['id'];?>" id="not_suspend">Mark to</a>
